...
#[repr(C)]
pub struct EfiBootServiciesTable {
    _reserved0: [u64; 7],
    ...
    _reserved1: [u64; 9],
    exit_boot_service: extern "win64" fn(image_handle: EfiHandle, map_key: usize) -> EfiStatus,
    _reserved4: [u64; 10],
...
const _: () = assert!(offset_of!(EfiBootServiciesTable, exit_boot_service) == 232);
...
impl EfiSystemTable {
    pub fn boot_servicies(&self) -> &EfiBootServiciesTable {
        self.boot_servicies
    }
...
pub fn locate_loaded_image_protocol(
    ...
    let status = (efi_system_table.boot_servicies.handle_protocol)(
        image_handle,
...
pub fn exit_from_efi_services(
    ...
    loop {
        let status = efi_system_table.boot_servicies.get_memory_map(memory_map);
        assert_eq!(status, EfiStatus::Success);
        let status =
            (efi_system_table.boot_servicies.exit_boot_service)(image_handle, memory_map.map_key);
        if status == EfiStatus::Success {
            break;
        }
    }
